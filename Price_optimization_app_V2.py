...
class DataManager:
    ...
    def get_elasticity_data(self) -> Dict[str, Any]:
        ...
        for product in self.data['product'].unique():
            ...
            if len(product_df) > 10:
                # Calculate price elasticity using log differences
                price_changes = product_df['price'].pct_change().dropna()
                demand_changes = product_df['demand'].pct_change().dropna()
                
                if len(price_changes) > 5 and price_changes.abs().sum() > 0.01:
                    # Elasticity = % change in demand / % change in price
                    elasticity = (demand_changes / price_changes).mean()
                else:
                    elasticity = -1.5  # Reasonable default
            else:
                elasticity = -1.5  # Default elasticity
            
            elasticity_data[product] = {
                'elasticity': float(elasticity) if not np.isnan(elasticity) else -1.5,
    ...
    def get_price_ranges(self) -> Dict[str, Any]:
        ...
        for product in self.data['product'].unique():
            ...
            ranges[product] = {
                'min_price': float(max(cost_price * 1.1, current_avg - price_std)),
                'max_price': float(current_avg + price_std * 1.5),
...
Ready for price optimization modeling!"""
    ...
    def _format_elasticity(self, elasticity: Dict) -> str:
        ...
        for product, stats in elasticity.items():
            ...
            analysis += f"  - Current Price: ${stats['avg_price']:.2f}\n"
            analysis += f"  - Current Demand: {stats['avg_demand']:.0f} units\n\n"
    ...
    def _format_price_ranges(self, ranges: Dict) -> str:
        ...
        for product, stats in ranges.items():
            ...
            analysis += f"  - 🏷️ Cost: ${stats['cost_price']:.2f}\n"
            margin = ((stats['current_avg'] - stats['cost_price']) / stats['cost_price'] * 100)
...
The historical data provides a solid foundation for price optimization modeling!"""
    ...
    def create_optimization_model(self, constraints: Dict = None) -> Dict[str, Any]:
        ...
        try:
            ...
            for product, data in price_ranges.items():
                ...
                current_demand = elasticity_data[product]['avg_demand']
                current_price = data['current_avg']
                
                # Linear approximation of profit function
                # profit ≈ (price - cost) * demand
                # where demand ≈ current_demand * (price/current_price)^elasticity
                
                # For linear programming, we use first-order approximation
                slope = elasticity * (current_demand / current_price)
                profit_approx = (current_price - cost) * current_demand + \
                              (current_demand + slope * (current_price - cost)) * (price_vars[product] - current_price)
                
                profit_terms.append(profit_approx)
            ...
            for product, data in price_ranges.items():
                ...
                prob += price_vars[product] >= data['cost_price'] * 1.1, f"Min_Margin_{product}"
            
            if constraints and 'max_total_price' in constraints:
                prob += pulp.lpSum([price_vars[product] for product in price_vars.keys()]) <= constraints['max_total_price'], "Total_Price_Limit"
    ...
    def solve_model(self, model_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for product in model_info['products']:
                ...
                current_price = model_info['price_ranges'][product]['current_avg']
                current_demand = model_info['elasticity_data'][product]['avg_demand']
                elasticity = model_info['elasticity_data'][product]['elasticity']
                ...
                results['demands'][product] = expected_demand
                results['profits'][product] = expected_profit
                results['changes'][product] = ((optimal_price - current_price) / current_price) * 100
                total_profit += expected_profit
            
            results['objective_value'] = total_profit
...
class ExplanationAgent:
    ...
    def explain_optimization_results(self, model_id: str) -> str:
        ...
        # Always use LLM if available, otherwise use enhanced rule-based
        if self.llm:
            try:
    ...
    def _explain_with_llm(self, model: Dict, results: Dict) -> str:
        """Use LLM for sophisticated natural language explanation"""
        prompt = f"""
        As a pricing optimization expert, explain these results to a business executive:
        
        OPTIMIZATION RESULTS:
        - Status: {results['status']}
        - Expected Total Profit: ${results['objective_value']:,.2f}
        - Number of Products: {len(model['products'])}
        
        OPTIMAL PRICING STRATEGY:
        {chr(10).join([f'- {product}: ${price:.2f} ({results["changes"][product]:+.1f}% change)' for product, price in results['prices'].items()])}
        
        Please provide a comprehensive explanation that covers:
        1. What the optimization achieved
        2. Key pricing recommendations  
        3. Business implications
        4. Risks and considerations
        5. Implementation suggestions
        
        Keep it professional yet accessible for business decision-makers.
        """
        
        messages = [
            SystemMessage(content="You are an experienced pricing strategist explaining optimization results to business executives. Provide clear, actionable insights."),
    ...
    def _explain_with_rules(self, model: Dict, results: Dict) -> str:
        ...
        explanation = "## 🎯 Price Optimization Results Analysis\n\n"
        
        # Executive Summary
        explanation += f"### 📊 Executive Summary\n"
        explanation += f"- **Optimization Status**: {results['status']}\n"
        explanation += f"- **Expected Profit Increase**: ${results['objective_value']:,.2f}\n"
        explanation += f"- **Products Optimized**: {len(model['products'])}\n\n"
        
        # Pricing Recommendations
        explanation += "### 💰 Recommended Pricing Strategy\n"
        
        # Sort products by price change magnitude
        sorted_products = sorted(results['changes'].items(), key=lambda x: abs(x[1]), reverse=True)
        
        for product, change_pct in sorted_products:
            optimal_price = results['prices'][product]
            current_price = model['price_ranges'][product]['current_avg']
            cost = model['price_ranges'][product]['cost_price']
            expected_demand = results['demands'][product]
            expected_profit = results['profits'][product]
            
            explanation += f"#### {product}\n"
            explanation += f"- **Recommended Price**: ${optimal_price:.2f} "
            explanation += f"({change_pct:+.1f}% from current ${current_price:.2f})\n"
            explanation += f"- **Expected Demand**: {expected_demand:.0f} units\n"
            explanation += f"- **Expected Profit**: ${expected_profit:,.0f}\n"
            explanation += f"- **Profit Margin**: {((optimal_price - cost) / optimal_price * 100):.1f}%\n"
            
            # Business interpretation
            if change_pct > 10:
                explanation += "  - 🚀 **Significant price increase recommended** - product may be undervalued\n"
            elif change_pct > 0:
                explanation += "  - 📈 **Moderate price increase** - balances revenue and volume\n"
            elif change_pct > -5:
                explanation += "  - ⚖️ **Minor price adjustment** - fine-tuning for optimal balance\n"
            else:
                explanation += "  - 📉 **Price decrease recommended** - to stimulate demand and increase volume\n"
            explanation += "\n"
        
        # Key Insights
        explanation += "### 🔍 Key Insights\n"
        explanation += "- **Profit Maximization**: Prices are optimized to balance margin and volume\n"
        explanation += "- **Elasticity Considered**: Recommendations account for price sensitivity\n"
        explanation += "- **Constraint Compliance**: All business constraints are respected\n"
        
        # Implementation Guidance
        explanation += "### 🚀 Implementation Strategy\n"
        explanation += "1. **Phase Implementation**: Roll out changes gradually\n"
        explanation += "2. **Monitor Performance**: Track actual vs. predicted results\n"
        explanation += "3. **Competitor Response**: Watch for competitive reactions\n"
        explanation += "4. **Customer Feedback**: Gather insights from the market\n"
        explanation += "5. **Regular Review**: Re-optimize quarterly or as market conditions change\n"
...
class MainOrchestrator:
    ...
    def _handle_model_creation(self, query: str) -> str:
        ...
        try:
            ...
            model_count = len(self.optimization_agent.models)
            
            return f"""✅ **Price Optimization Model Created Successfully!**
...
The model is ready for optimization. Use 'solve {model_id}' to find optimal prices."""
    ...
    def _handle_solve_request(self, query: str) -> str:
        ...
        prices_text = "\n".join([f"- {product}: ${price:.2f} ({results['changes'][product]:+.1f}%)" 
...
Ask me to 'explain the results' for a detailed business analysis."""
    ...
    def _general_conversation(self, query: str) -> str:
        ...
        return """I'm your AI price optimization assistant! I can help you:

📊 **Analyze Historical Data** - Understand sales patterns and price elasticity
⚡ **Create Optimization Models** - Build mathematical models for pricing  
🎯 **Solve for Optimal Prices** - Find profit-maximizing price points
💡 **Explain Results** - Get business insights and implementation guidance
...
I'll guide you through the complete price optimization workflow!"""
...
# Streamlit UI
def main():
    st.title("💰 AI Price Optimization System")
    st.markdown("**Multi-Agent System: Data Analysis → Optimization → Business Insights**")
    
    # Initialize all components in session state to maintain consistency
    if 'data_manager' not in st.session_state:
        st.session_state.data_manager = DataManager()
    if 'data_agent' not in st.session_state:
        st.session_state.data_agent = DataAnalysisAgent(st.session_state.data_manager)
    if 'optimization_agent' not in st.session_state:
        st.session_state.optimization_agent = PriceOptimizationAgent(st.session_state.data_manager)
    if 'explanation_agent' not in st.session_state:
        st.session_state.explanation_agent = ExplanationAgent(st.session_state.optimization_agent)
    if 'orchestrator' not in st.session_state:
        st.session_state.orchestrator = MainOrchestrator(
            st.session_state.data_agent, 
            st.session_state.optimization_agent, 
            st.session_state.explanation_agent
        )
    if 'workflow' not in st.session_state:
        st.session_state.workflow = create_agent_workflow(st.session_state.orchestrator)
    if 'conversation' not in st.session_state:
        st.session_state.conversation = []
    if 'agent_handovers' not in st.session_state:
        st.session_state.agent_handovers = []
    
    # Use the agents from session state
    data_manager = st.session_state.data_manager
    optimization_agent = st.session_state.optimization_agent
    ...
    with st.sidebar:
        ...
        # Model status - THIS IS THE KEY FIX
        model_count = len(optimization_agent.models)
        st.header("📊 Model Status")
        if model_count > 0:
            st.success(f"✅ **{model_count} Model(s) Active**")
            for model_id, model in optimization_agent.models.items():
                status_icon = "✅" if model.get('status') == 'solved' else "⏳"
                with st.expander(f"{status_icon} {model_id}", expanded=False):
                    st.write(f"**Status**: {model.get('status', 'created')}")
                    st.write(f"**Products**: {len(model.get('products', []))}")
                    if 'results' in model:
                        st.write(f"**Profit**: ${model['results'].get('objective_value', 0):,.0f}")
        else:
        ...
        if st.session_state.agent_handovers:
            for i, (query, agent, reason) in enumerate(st.session_state.agent_handovers[-5:]):
                with st.expander(f"Step {i+1}: {agent}", expanded=False):
    ...
    with col2:
        ...
        # Proper model display using the same instance
        model_count = len(optimization_agent.models)
        
        if model_count > 0:
            st.success(f"✅ **{model_count} Active Model(s)**")
            
            for model_id, model in optimization_agent.models.items():
                with st.expander(f"🎯 {model_id} - {model.get('status', 'unknown')}", expanded=False):
                    
                    st.write(f"**Products**: {', '.join(model.get('products', []))}")
                    st.write(f"**Type**: {model.get('model_type', 'N/A')}")
                    
                    if 'results' in model:
                        results = model['results']
                        st.success(f"**Solved**: Profit = ${results.get('objective_value', 0):,.0f}")
                        
                        st.write("**Optimal Prices:**")
                        for product in model.get('products', []):
                            if product in results.get('prices', {}):
                                ...
                                change = results['changes'][product]
                                st.write(f"- {product}: ${price:.2f} ({change:+.1f}%)")
                    else:
                        st.info("Model created but not solved yet")
                    
                    # Action buttons
                    col1, col2 = st.columns(2)
                    with col1:
                        if st.button("Solve", key=f"solve_{model_id}"):
                            st.session_state.conversation.append((f"solve {model_id}", "", "pending", ""))
                            st.rerun()
                    with col2:
                        if st.button("Explain", key=f"explain_{model_id}"):
                            st.session_state.conversation.append((f"explain {model_id}", "", "pending", ""))
                            st.rerun()
        else:
            st.info("👆 Create your first model using the 'Create Model' button above!")
